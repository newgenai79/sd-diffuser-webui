...
def get_pipeline(memory_optimization, vaeslicing, vaetiling):
    ...
    if (modules.util.appstate.global_pipe is not None and 
        type(modules.util.appstate.global_pipe).__name__ == "CogVideoXPipeline" and
    ...
    modules.util.appstate.global_pipe = CogVideoXPipeline.from_pretrained(
...
def create_cogvideox155b_t2v_tab():
    initial_state = state_manager.get_state("cogvideox155b_t2v") or {}
    with gr.Row():
        with gr.Column():
        ...
        with gr.Column():
            cogvideox155b_vaeslicing = gr.Checkbox(label="VAE Slicing", value=initial_state.get("vaeslicing", True), interactive=True)
            cogvideox155b_vaetiling = gr.Checkbox(label="VAE Tiling", value=initial_state.get("vaetiling", True), interactive=True)
    with gr.Row():
        with gr.Column():
        ...
        with gr.Column():
            ...
            with gr.Row():
                ...
                cogvideox155b_guidance_scale = gr.Slider(label="Guidance Scale", minimum=1, maximum=20, step=0.1, value=initial_state.get("guidance_scale", 6.0))
    with gr.Row():
    ...
    def save_current_state(memory_optimization, vaeslicing, vaetiling, width, height, fps, inference_steps, no_of_frames, guidance_scale):
        ...
        initial_state = state_manager.get_state("cogvideox155b_t2v") or {}
        return state_manager.save_state("cogvideox155b_t2v", state_dict)
    ...
    save_state_button.click(
        ...
        inputs=[
            cogvideox155b_memory_optimization, 
            cogvideox155b_vaeslicing,
            cogvideox155b_vaetiling,
            cogvideox155b_width_input, 
            cogvideox155b_height_input, 
            cogvideox155b_fps_input, 
            cogvideox155b_num_inference_steps_input,
            cogvideox155b_num_frames_input,
