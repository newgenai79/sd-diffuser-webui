...
lora_path = "models/lora/flux.1-schnell/"
...
def get_pipeline(model_type, memory_optimization, lora, lora_weight, bypass_token_limit, performance_optimization):
    print("----Flux-schnell mode: ", model_type, memory_optimization, lora, lora_weight, bypass_token_limit, performance_optimization)
     ...
     type(modules.util.appstate.global_pipe).__name__ == "FluxPipeline" and 
      modules.util.appstate.global_model_type == model_type and
       modules.util.appstate.global_bypass_token_limit == bypass_token_limit and 
        ...
        print(">>>>Reusing Flux Schnell pipe<<<<")
...
def generate_images(
    ...
    try:
        ...
        for img_idx in range(no_of_images):
            ...
            if (lora):
                metadata["lora"] = lora
                metadata["lora_weight"] = lora_weight
            # Save the image
...
def create_flux_schnell_tab():
    ...
    with gr.Row():
        with gr.Column():
            ...
            with gr.Row():
                flux_lora = gr.Dropdown(
                    choices=get_lora_files(),
                    label="Select LoRA",
                    value=initial_state.get("lora", None),
                    interactive=True
                )
                lora_refresh_btn = gr.Button("🔄", elem_classes="small-button")
            with gr.Row():
                flux_lora_weight_input = gr.Textbox(
                    label="Weight", 
                    lines=1,
                    value=1.0,
                    interactive=True
