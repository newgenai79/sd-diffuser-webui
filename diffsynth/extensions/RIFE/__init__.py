...
class RIFEInterpolater:
    ...
    def interpolate(self, images, scale=1.0, batch_size=4, num_iter=1, progress_bar=lambda x:x):
        ...
        # Calculate total iterations for accurate progress tracking
        total_iters = num_iter
        frames_per_iter = len(processed_images) - 1
        batches_per_iter = (frames_per_iter + batch_size - 1) // batch_size  # Ceiling division
        total_batches = total_iters * batches_per_iter
        current_batch = 0
    ...
    def interpolate(self, images, scale=1.0, batch_size=4, num_iter=1, progress_bar=lambda x:x):
        ...
        # Calculate total work across all iterations
        total_work = 0
        frames_after_iter = len(processed_images)
        
        for i in range(num_iter):
            frames_to_process = frames_after_iter - 1
            total_work += frames_to_process
            frames_after_iter = frames_after_iter * 2 - 1  # Each iteration doubles frames minus 1
        
        completed_work = 0
        
        for iter in range(num_iter):
            # Calculate number of frame pairs in this iteration
            frame_pairs = len(processed_images) - 1
            ...
            for batch_id in range(0, input_tensor.shape[0], batch_size):
                batch_id_ = min(batch_id + batch_size, input_tensor.shape[0])
                batch_count = batch_id_ - batch_id
                ...
                progress = min(completed_work / total_work, 1.0)
                progress_bar(progress)
